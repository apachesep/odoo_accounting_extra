...
class account_invoice(osv.osv):
    ...
    # compute what the new orders should contain
        ...
        for porder in [order for order in self.browse(cr, uid, ids, context=context) if order.state == 'draft']:
            order_key = make_key(porder, ('partner_id',  'type', 'account_id', 'currency_id', 'journal_id', 'company_id'))#'user_id',
            ...
            if not order_infos:
                ...
                if porder.user_id.id <> order_infos['user_id']:
                    order_infos['user_id'] = False
...
#            order_data['invoice_line'] = [(0, 0, value) for value in order_data['invoice_line'].itervalues()]
        ...
        for porder in [order for order in self.browse(cr, uid, ids, context=context) if order.state == 'cancel']:
            for inv_line in porder.invoice_line:
                inv_line_obj.write( cr, uid, [inv_line.id], {'invoice_id': neworder_id})
