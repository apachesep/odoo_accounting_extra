...
class rml_parse(report_sxw.rml_parse):
    def __init__(self, cr, uid, name, context):
        ...
        self.localcontext.update({
            ...
            'movelines' :self._get_moveline,
        })

    # Wang zhenzhen add this function
    def _get_exchange_rate(self, line):
        '''
        Exchange rate: Debit or Credit / currency ammount
        Why not get it from currency code + date ?
        '''
        # This method has been overrided!
        pass

        

    def _get_moveline(self, lines):
        line = [(l.name, self._get_account_name(l.account_id.id), l.credit, l.debit, l.amount_currency,l.amount_currency and l.currency_id and l.currency_id.symbol or '', l.reconcile_partial_id and 'v' or l.reconcile_id and 'V' or '',l.currency_id and self._get_exchange_rate(l) or '', l.partner_id and l.partner_id.name or '') for l in lines]
        for t in range(0, 8 - (len(line) % 8)):
    ...
    def _rmb_format(self, value):
        ...
        if value != 0:
            for i in  range(9, -3, -1):
                if value >= 10**i or i < 1:
                    nums[j]=str(int(round(value/(10**i),2))%10)
                j=j+1
        return nums
    ...
    def _get_account_name(self,id):
        if not id:
            return ''
        account_name = self.pool.get('account.account').browse(self.cr, self.uid, [id],{})[0]
        #Account move print use Account here:
        return account_name.code  + account_name.name+'\n'
